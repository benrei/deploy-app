...
const deploy = (deploy) => {
  if(!deploy.user || !deploy.host || !deploy.path) console.log("  NB! 'user', 'host' and 'path' are required.");
    ...
    const cmdPreDeploy = deploy['pre-deploy-local']
      ? `'${deploy['pre-deploy-local']}; exit;'`
      : null;
    const cmdPostDeploy = deploy['post-deploy']
      ? `'cd ${deploy.path}; ${deploy['post-deploy']}; exit;'`
      : null;

    ssh.run(deploy.user,deploy.host,deploy.port, cmdPreDeploy,'pre-deploy-local',(callback)=>{
      scp.send(deploy.user,deploy.host,deploy.port,deploy.files,deploy.path, (callback)=>{
        ssh.run(deploy.user,deploy.host,deploy.port,cmdPostDeploy,'post-deploy', (callback) =>{
          console.log('Our job is done!')
        });
      });
    });
